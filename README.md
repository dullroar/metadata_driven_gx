...
| `end_to_end.py` | Runs the three stages above in order, in one command or from a notebook. |
...
| `dbx_notebooks/end_to_end.ipynb` | A committed, guided notebook that runs the worked demo through the same pipeline as the CLI. |

The repository also includes a notebook-first introduction in
[`dbx_notebooks/README.md`](dbx_notebooks/README.md). It is aimed at readers who want to
run the complete example interactively in about ten minutes. The committed notebook is
teaching material; `play/` notebooks generated by `edit_suite.py` remain disposable
scratchpads for editing one suite.
...
[`dbx_notebooks/end_to_end.ipynb`](dbx_notebooks/end_to_end.ipynb) runs the committed
`demo_weather` environment, displays representative validation results, and shows where
GX Data Docs are written. It calls `end_to_end.run_pipeline()`, the same orchestration
used by the command-line entry point, so the notebook is an alternate view of the code,
...
end_to_end.py                    runs the above in order (CLI and reusable notebook API)
edit_suite.py                    generates a disposable notebook for interactively editing one suite
dbx_notebooks/                   committed, guided notebooks and notebook-focused README
